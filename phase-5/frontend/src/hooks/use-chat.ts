...
import { chatApi, HistoryMessage, ChatResponse, ApiError, InputMethod } from "@/lib/chat-api";
...
export function useChat(options: UseChatOptions = {}) {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async ({ message, inputMethod }: { message: string; inputMethod: InputMethod }) => {
      if (!userId) {
        throw new Error("User not authenticated");
      }
      return chatApi.sendMessage(userId, message, inputMethod);
    },
    onMutate: async ({ message, inputMethod }) => {
      ...
      return { tempId, message, inputMethod };
    },
  ...
  const sendMessage = useCallback(
    async (message: string, inputMethod: InputMethod = "text") => {
      ...
      await sendMessageMutation.mutateAsync({ message, inputMethod });
    },
