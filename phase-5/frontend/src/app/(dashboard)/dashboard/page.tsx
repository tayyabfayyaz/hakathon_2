...
export default function DashboardPage() {
  ...
  const removeDeadline = useRemoveDeadline();
  ...
  const handleDeleteDialogChange = useCallback((open: boolean) => {
    ...
    if (!open) {
      // Delay clearing to avoid issues with the mutation
      setTimeout(() => setTaskToDelete(null), 100);
    }
  }, []);
  ...
  const handleSetDeadline = useCallback((id: string, deadline: string) => {
    setDeadline.mutate({ id, deadline });
  }, [setDeadline]);

  const handleRemoveDeadline = useCallback((id: string) => {
    removeDeadline.mutate(id);
  }, [removeDeadline]);

  const completedCount = tasks.filter((t) => t.completed).length;
  const totalCount = tasks.length;
  const tasksWithDeadline = tasks.filter((t) => t.deadline && !t.completed).length;
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-3xl text-blue-600">{tasksWithDeadline}</CardTitle>
          </CardHeader>
        </Card>
      </div>
