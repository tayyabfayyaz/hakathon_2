// Use proxy route for browser requests (K8s deployment compatibility)
// Server-side can use internal K8s DNS, browser must use proxy
function getApiBase(): string {
  if (typeof window === "undefined") {
    // Server-side: use internal K8s service name
    return process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
  }
  // Browser: use proxy route to avoid K8s DNS resolution issues
  return "/api/proxy";
}
